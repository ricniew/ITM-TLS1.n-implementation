...
**Note**: If you are not logged in on the HUB TEMS Host, you need to set also the variables listed below (for Unix/Linux change "set" to "export") before running `kdstsns` tool. However the program `kdstsns` must be installed on that host:
    ...
    set KDC_PORTS=(2 135 3661)                   # for ip.pipe use port 1918, on Linux/Unix use KDC_PORTS="(2 135 3661)" 
...
- Your environment **must be at least at ITM 6.3 FP7 SP05 (or higher)** and a **WAS 855 uplift must have been performed** before implementing TLSv1.2. The WAS 855 uplift is described in the SP update readme files. To check if a WAS uplift was made use ITMHOME/[arch]/iw/bin/versionInfo.sh or ITMHOME\CNPSJ\bin\versionInfo.bat. The version must be at least 8.5.5.16
...
- For Windows: [ZIP format](https://github.com/ricniew/ITM-TLS1.n-implementation/archive/2.33.zip) 
- For Unix/linux: [TAR format](https://github.com/ricniew/ITM-TLS1.n-implementation/archive/2.33.tar.gz) 
