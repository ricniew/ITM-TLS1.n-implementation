...
saveorgcreatenew () 
{
  ...
  #echo  "DEBUG - saveorgcreatenew - new= $NEWORGFILE save= $SAVEORGFILE org= $orgfile" ; exit
  if [ ! -f $orgfile ] ; then
      echo  "WARNING - saveorgcreatenew - $orgfile does not exists. Component apparently not installed"
      return 1
  fi
...
renewCert () 
{
  ...
  CH=$CANDLEHOME
  IWDIR=$(ls -d $CANDLEHOME/[al]*/iw 2> /dev/null)
...
if [ -d "${CANDLEHOME}/${BACKUPFOLDER}" ]; then
    echo "ERROR - main - Backup directory ${CANDLEHOME}/${BACKUPFOLDER} exists already. Please check!"
    usage
    exit 1
else
