...
function backupfile ($file) {
  ...
  if ( test-path $file ) {
      ...
      Copy-Item -Path "$file" -Destination $BACKUPFOLDER -erroraction stop
      if ( -not $? ) {
...
function createRestoreScript ($restorebat) {
  ...
  $null = New-Item -Path $BACKUPFOLDER -Name $restorebat -ItemType "file"
  ...
  foreach ( $h in $HFILES.Keys ) {
      ...
      if ( ( $h -eq "kcjparms.txt" ) -and ( $KCJ = 4 ) ) { 
  ...
  Add-Content $restorebatfull " "
  Add-Content $restorebatfull "kinconfg -n -rKCW"
...
function saveorgcreatenew ($orgfile) {
  ...
  if ( -Not (test-path "$orgfile") ) { 
      write-host "WARNING - saveorgcreatenew - $orgfile does not exists. Component apparently not installed"
      $return.rc = 1
      return $return
  }
...
function restartTEPS () {
  ...
  if ( -not $? ) {
      ...
      write-host "INFO - restartTEPS - TEPS restarted successfully."
...
if ( test-path $BACKUPFOLDER ) { 
    write-host "ERROR - main - Folder $BACKUPFOLDER exists already! Please check and restart"
    exit 1
} else {
...
if ( -not $? ) {
    ...
    write-host "INFO - main - Successfully executed gsk8capicmd  commands to copy renewed certificates to $KEYKDB" 
}
...
write-host "INFO - main - Reconfiguring KCW"
kinconfg -n -rKCW
if ( -not $? ) {
